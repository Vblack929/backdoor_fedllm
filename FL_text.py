...
from utils import get_dataset, get_attack_test_set, get_attack_syn_set, get_clean_syn_set, average_weights, exp_details, extract_lora_matrices, compute_stats, detect_anomalies_with_kde, load_params
...
if __name__ == '__main__':
    ...
    class Args:
        def __init__(self):
            ...
            self.epochs = 15 # Number of rounds of training
            ...
            # Portion of compromised clients in classic Backdoor attack against FL
            self.attackers = 0.3
    ...
    BD_users = np.random.choice(
        np.arange(args.num_users), num_attackers, replace=False)
    print(f"Compromised users: {BD_users}")
    for epoch in tqdm(range(args.epochs)):
